...
namespace Skincare.Services.Implements
{
    public class FileService : IFileService
    {
        ...
        public async Task<string?> SaveFileAsync(IFormFile? file, string folderName)
        {
            ...
            {
                _logger.LogWarning("SaveFileAsync called with null or empty file for folder: {Folder}", folderName);
                return null;
            }

            try
            {
                _logger.LogInformation("Starting file upload: Name={FileName}, Size={FileSize}KB, Type={ContentType}, Folder={Folder}", 
                    file.FileName, file.Length / 1024, file.ContentType, folderName);
                
                var fileExtension = Path.GetExtension(file.FileName);
                var fileName = $"{Guid.NewGuid()}{fileExtension}";
                var uploadDir = Path.Combine(_uploadPath, folderName);

                _logger.LogInformation("Upload directory: {UploadDir}, full path exists: {Exists}", 
                    uploadDir, Directory.Exists(uploadDir));
