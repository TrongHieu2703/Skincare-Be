...
namespace Skincare.Repositories.Implements
{
    public class InventoryRepository : IInventoryRepository
    {
        ...
        {
            return await _context.Inventories.Include(i => i.Product).FirstOrDefaultAsync(i => i.Id == id);
        }

        public async Task<bool> CheckStockAvailability(int productId, int quantity)
        {
            var stock = await _context.Inventories.Where(i => i.ProductId == productId).SumAsync(i => i.Quantity) ?? 0;
